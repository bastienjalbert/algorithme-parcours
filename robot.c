...
{
  Case case_a_tester;
  //printf("Marquée ? ligne --> %d colonne --> %d\n", a_verifier->coord.num_ligne, a_verifier->coord.num_col);
  for (int i = 1; i <= Taille_liste_dynamique_generique(marquees); i++) {
      ...
      //printf("Déjà marquée : ligne --> %d colonne --> %d\n", case_a_tester.coord.num_ligne, case_a_tester.coord.num_col);
...
void etat_suivants(Etape * etape_a_traiter, Liste_dynamique_generique *traitement,
   Liste_dynamique_generique *marquees, Problem *probleme) {
     ...
     Etape haut = creer_etape(recuperer_case(probleme, cur_lin - 1, cur_col), etape_a_traiter);
     Etape bas = creer_etape(recuperer_case(probleme, cur_lin + 1, cur_col), etape_a_traiter);
     Etape gauche = creer_etape(recuperer_case(probleme, cur_lin, cur_col - 1), etape_a_traiter);
     Etape droite = creer_etape(recuperer_case(probleme, cur_lin, cur_col + 1), etape_a_traiter);
